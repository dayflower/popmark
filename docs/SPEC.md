...
The rich editor is built on the **vendored `LexicalMarkdownEditor` component** (`src/lexical-markdown/`, copied from the sibling [`etude-lexical-markdown`](https://github.com/dayflower/etude-lexical-markdown) project). It is a controlled Markdown component: it takes a `value` (Markdown string) and emits `onChange(markdown)`; Markdown↔Lexical conversion, the custom link/code-block nodes, bare-URL auto-link decoration, blockquote/list/task-list behavior, and Prism-based code highlighting all live inside it. The vendored copy tracks upstream (currently v0.6.0): it exposes an `editorRef` prop for the live `LexicalEditor` (used for clipboard export and focus control) and a headless `markdownToHtml` helper whose custom nodes emit clean semantic HTML. popmark applies a local patch to the vendored copy (commented `popmark patch`): both explicit-link and auto-link opening are routed through the Tauri opener plugin instead of `window.open`.

As of v0.4.0 the component also: opens an explicit link's URL on `cmd`/`ctrl`+click (previously only auto-links and clicks on the focused URL did); toggles a `data-mod-pressed` attribute on the editor root while a `cmd`/`ctrl` modifier is held, which popmark's host CSS keys a pointer cursor off so a hovered link reads as clickable; shows the link destination as a native `title` tooltip while the link is rendered (stripped while focused, where the literal markdown is shown); and clears inline formatting (`code`/`bold`/`italic`/`strikethrough`) when the caret rests on the outer boundary of such a run, so typing there is no longer trapped inside the format.

As of v0.5.0 the component exposes a `linkClickBehavior` prop (`"edit"` | `"open"`, default `"edit"`) that swaps which mouse gesture opens a link's URL vs. edits it, for both explicit and auto-links. With `"edit"` (popmark's default, unchanged behavior) a plain click edits and `cmd`/`ctrl`+click opens; with `"open"` a plain click opens (guarded so a drag or text selection across a link never opens it) and `cmd`/`ctrl`+click edits. The `data-mod-pressed` pointer-cursor hint now tracks whichever gesture is armed for the active mode.
